...
- **🌐 多源聚合**：36 氪 AI 频道列表抓取 + GitHub Search API，按「Agent 与工作流 / Skill / AI 应用」三类 topic 并行检索
- **🤖 大模型筛选总结**：新闻侧筛选 **Top 5** 并生成短摘要；项目侧在候选集中由模型统一去重归类、翻译中文项目名并写要点
- **📈 新库热度信号**：优先近 **14 天**创建、按 **Star 增速（★/天）**排序后送入模型，突出新爆发项目而非单纯总星数
- **📱 飞书富文本**：`post` 消息类型，分区标题 + **可点击标题链接** + 摘要，群聊里直接跳转原文与仓库
- **⚡ 全自动流程**：通过GitHub Actions 设置定时任务

## 🚀 快速开始

### 环境要求
- Python 3.10+（GitHub Actions 与 `.github/workflows/schedule.yml` 中 `setup-python` 一致为 **3.11**）
- 一个 LLM API Key
- 一个飞书机器人 Webhook
